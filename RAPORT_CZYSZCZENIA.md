...
Wynikowa baza ma 346 plików i uproszczony układ: [`c1`](c1)–[`c6`](c6), [`Wiedza`](Wiedza), [`Moze_potrzebne`](Moze_potrzebne) oraz ten raport.
...
- `c1.zip`, ponieważ zawierał kopię katalogu [`c1`](c1).

## Jak pogrupowano materiały

- [`c1`](c1) — materiały o terminalu, SSH, kluczach, pracy lokalnej/zdalnej i podstawach Uniksa.
- [`c2`](c2) — materiały o modelu OSI, interfejsach, MAC, IPv4/IPv6, CIDR i analizie sieci.
- [`c3`](c3) — DHCP, ARP, ICMP, IPv6 oraz zrzuty ruchu; zachowano pliki `.pcap`/`.pcapng` jako dane ćwiczeniowe.
- [`c4`](c4) — DNS, mDNS, LLMNR, NBNS, trasowanie, przekazywanie pakietów i instrukcja Azure Linux VM.
- [`c5`](c5) — VPS, chmura, sieć ADM, ZeroTier/Tailscale i automatyzacja skryptami.
- [`c6`](c6) — zapory, Docker/Incus, usługi w chmurze i podłączanie serwera do sieci wirtualnej.
- [`Wiedza`](Wiedza) — wykłady, ściągi, praca w labie, Wireshark, IPv6, sprawozdania, skrypty i książki.
- [`Moze_potrzebne`](Moze_potrzebne) — projekty indywidualne, materiały anglojęzyczne/akademickie z Berkeley, dokumenty Office oraz pliki o niepewnym przypisaniu.

## Pliki scalone do README

Scalono i przepisano do spójnych notatek:

- `c1/Cel-ćwiczenia.txt`, `c1/Plan-pracy.txt`, `c1/Pytania*.txt`, `c1/Unix-podstawy.txt`, `Praca-lokalna.txt`, `Praca-zdalna.txt`, `Reperacja-sieci.txt`, `Reperacja-grafiki.txt` → [`c1/README.md`](c1/README.md) i [`Wiedza/Praca_w_labie/README.md`](Wiedza/Praca_w_labie/README.md).
- `c2/Cel-ćwiczenia.txt`, `c2/Pytania.txt`, `c2/c2-zadanie.txt` oraz wybrane diagramy z `old/25/c2` → [`c2/README.md`](c2/README.md).
- `c3/Cel-ćwiczenia.txt`, `c3/Pytania.txt`, skrypty `arp-zrzut`, `dhcp-zrzut` i materiały DHCP → [`c3/README.md`](c3/README.md) z zachowaniem skryptów i zrzutów.
- `c4/Cel-ćwiczenia.txt`, [`c4/DNS.md`](c4/DNS.md), `c4/Przekazywanie-Linux.txt`, `c4/mDNS*.txt`, `c4/vm-student.md` → [`c4/README.md`](c4/README.md), [`c4/DNS.md`](c4/DNS.md), [`c4/azure-linux-vm.md`](c4/azure-linux-vm.md).
- `c5/Cel-ćwiczenia.txt`, `c5/zadanie.txt`, `c5/google-cloud-doc.txt` → [`c5/README.md`](c5/README.md) oraz [`c5/materialy/google-cloud.md`](c5/materialy/google-cloud.md).
- `c6/Cel-ćwiczenia.txt` i wybrane aktualne/stare materiały `old/25/c6` → [`c6/README.md`](c6/README.md) i [`c6/materialy/`](c6/materialy/).
- Linki `.url`/`.webloc` scalono do plików `linki.md` w odpowiednich katalogach.
- `Egzamin/Przyładowe-pytania.pdf` przepisano do [`Wiedza/Egzamin/README.md`](Wiedza/Egzamin/README.md) i zachowano oryginalny PDF.
...
- Część materiałów o NAT, Wi-Fi i mostkach pojawiała się w różnych rocznikach pod różnymi numerami ćwiczeń. Zamiast przypisywać je agresywnie do jednego `cX`, materiały ogólne przeniesiono do [`Wiedza`](Wiedza), a mniej pewne do [`Moze_potrzebne`](Moze_potrzebne).
- Projekty indywidualne i obce materiały akademickie są potencjalnie przydatne, ale nie są rdzeniem bieżących laboratoriów, więc trafiły do [`Moze_potrzebne`](Moze_potrzebne).
...
- PDF-y z Berkeley CS168 i prezentacje `.pptx` — przydatne jako materiały dodatkowe, ale anglojęzyczne i spoza głównego polskiego toku laboratoriów; przeniesione do [`Moze_potrzebne`](Moze_potrzebne).
...
Pełny techniczny wykaz PDF-ów znajduje się w [`Wiedza/PDF_PRZEGLAD.md`](Wiedza/PDF_PRZEGLAD.md).

## Pliki nieprzetworzone w pełni

Nie przerabiano w pełni bardzo dużych podręczników, zewnętrznych wykładów anglojęzycznych, plików Office z Berkeley oraz starych roczników, jeśli nie były konieczne do aktualnej bazy. Zostały zachowane w [`Wiedza/Ksiazki`](Wiedza/Ksiazki) albo [`Moze_potrzebne`](Moze_potrzebne), jeśli mogły mieć wartość informacyjną.
